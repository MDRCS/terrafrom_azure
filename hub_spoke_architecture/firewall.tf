...
# resource "azurerm_resource_group" "az-firewall-rg" {
#   name     = "${local.prefix}-rg"
#   location = local.fw_location
...
#   resource_group_name = azurerm_resource_group.az-firewall-rg.name
#   location            = azurerm_resource_group.az-firewall-rg.location
# }
...
#   #   # public_ip_address_id = azurerm_public_ip.fw-mgmt-ip.id # Because of the limit of 4 public ip that i can create
#   # }
...
#     rule {
#       name                  = "allow_network_to_laptop"
#       protocols             = ["TCP", "UDP", "ICMP"]
#       source_addresses      = ["*"]
#       destination_addresses = [data.external.my_ip.result.ip]
#       destination_ports     = ["*"]
#     }
